                    ...
                    case 3://Clientes cumplidores
                        {
                            ...
                            SqlCommand consulta = Database.consultaCrear("select distinct top 5 CONCAT(Domicilio_Ciudad,' ',Domicilio_Calle,' ',Domicilio_NumeroCalle) 'Hotel',Habitacion_Numero,DATEDIFF(day,Estadia_FechaInicio,Estadia_FechaFin)'Dias utilizados',(select COUNT(ReservaHabitacion_HabitacionID)'Cantidad de veces utilizadas' from rip.Reservas_Habitaciones where a.ReservaHabitacion_HabitacionID=ReservaHabitacion_HabitacionID group by ReservaHabitacion_HabitacionID)'Cantidad de veces utilizadas' from rip.Reservas join rip.Reservas_Habitaciones a on Reserva_ID=ReservaHabitacion_ReservaID join rip.Habitaciones on ReservaHabitacion_HabitacionID=Habitacion_ID join rip.Hoteles on Habitacion_HotelID=Hotel_ID join rip.Domicilios on Domicilio_ID=Hotel_DomicilioID join rip.Estadias on Estadia_ReservaID=Reserva_ID where YEAR(Estadia_FechaInicio)=@anio and DATEPART(QUARTER,Estadia_FechaInicio)=@trimestre  group by Domicilio_Ciudad,Domicilio_Calle,habitacion_Numero,Domicilio_NumeroCalle,Estadia_FechaInicio,Estadia_FechaFin,a.ReservaHabitacion_HabitacionID order by 2 desc ,3 desc ");
                            consulta.Parameters.AddWithValue("@anio", anio);
                            consulta.Parameters.AddWithValue("@trimestre", trimestre);
                            dataGridViewCargar(dataGridEstadisticas,Database.consultaObtenerTabla(consulta));
                        };
